...
- **Validation pipeline** &mdash; Opt-in to `ValidationViewModelBase` to compose validation rules (such as `Required` or custom `Validate` callbacks) that keep the `Errors` collection and `HasErrors` flag in sync with your UI.
...
`FluentSetterViewModelBase` provides the `When` helper to build rich property setters without repetitive boilerplate:
...
### Validation
Derive from `ValidationViewModelBase` to wire validation rules directly into your setters:
